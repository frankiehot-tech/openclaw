...
def convert_to_integrated_hetu_state(legacy_state: t.Any) -> HetuState:
    ...
    # 通过状态名称进行转换
    state_name = (
        legacy_state.name if hasattr(legacy_state, "name") else str(legacy_state)
    )
...
class HexagramEnhancedLuoshuScheduler(HetuLuoshuScheduler):  # type: ignore[misc]
    ...
    def submit_task(
        ...
        """提交评估任务（增强版）"""
        # 调用父类方法创建任务
        task_id: str = super().submit_task(
            code, task_type, priority, context, test_cases
        )
        ...
        hetu_state = self.hexagram_adapter.hexagram_manager.get_hetu_state(
            hexagram_state
        )
        if hetu_state:
    ...
    def execute_task(self, task_id: str) -> bool:
        ...
        # 执行状态转移
        success = self.hexagram_adapter.transition(
            task_id, current_hetu_state, target_hetu_state
        )
        ...
        # 执行调度（模拟成功，因为状态转移已通过适配器完成）
        # 注意：在真实系统中，这里会执行实际的评估任务
        # 但在模拟器中，我们假设调度执行成功
        print("   调度执行: 模拟成功 (状态转移已完成)")
        ...
        if new_hexagram and new_hexagram != current_hexagram:
            new_hexagram_name = (
                self.hexagram_adapter.hexagram_manager.get_hexagram_name(new_hexagram)
            )
        ...
        base_report["hexagram_adapter"] = {
            ...
            "hexagram_manager": {
                ...
                "state_history_count": len(
                    self.hexagram_adapter.hexagram_manager.state_history
                ),
            },
        }
    ...
    def _collect_hexagram_statistics(self) -> t.Dict[str, t.Any]:
        ...
        for task_id in self.tasks:
            ...
            if analysis:
                # 卦象分布
                hexagram_name = analysis.hexagram_name
                hexagram_counts[hexagram_name] = (
                    hexagram_counts.get(hexagram_name, 0) + 1
                )
