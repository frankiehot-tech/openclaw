...
def categorize_pending_tasks(pending_tasks):
    ...
    for task_id, task_details in pending_tasks:
        ...
        stage = task_details.get('stage', '').lower()

        # 分类逻辑
        if any(keyword in title for keyword in ['验证', 'validation', '审计', 'audit', '收口', 'closeout', '风险', 'risk']):
        ...
        elif any(keyword in title for keyword in ['执行', 'execution', '框架', 'harness', 'runner']):
...
def main():
    ...
    parser.add_argument('--target-completion', type=float, default=90.0,
                       help='目标完成度百分比 (默认: 90.0)')
    parser.add_argument('--dry-run', action='store_true',
                       help='试运行模式，不实际修改队列文件')
    parser.add_argument('--mark-all-old', action='store_true',
                       help='标记所有超过3天的pending任务为completed')
        ...
        if changes_made > 0:
            ...
            if save_queue_data(queue_data):
                ...
                if new_completion >= args.target_completion:
                    ...
                    print(f"   📈 仍需努力，距离{args.target_completion}%阈值还差{args.target_completion-new_completion:.1f}%")
            else:
                print("❌ 保存队列文件失败")
        else:
            print("ℹ️ 没有做出任何修改")
