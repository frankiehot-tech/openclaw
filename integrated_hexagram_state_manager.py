...
class IntegratedHexagramStateManager:
    ...
    def _load_mappings(self) -> list[HexagramMapping]:
        ...
        for item in data.get("mappings", []):
            ...
            if hetu_state is None:
                raise ValueError(f"未知河图状态: {hetu_state_name} ({hetu_state_value})")
    ...
    def transition(
        ...
        print(f"   河图状态: {from_mapping.hetu_state.name} → {to_mapping.hetu_state.name}")
        print(f"   原因: {reason}")
    ...
    def visualize_state_space(self, highlight_hetu_states: list[HetuState] | None = None) -> str:
        ...
        if self.current_state:
            ...
            if analysis:
                lines.append(f"   卦象: {analysis.hexagram_name} ({self.current_state})")
                lines.append(f"   河图: {analysis.hetu_state_name} ({analysis.hetu_state.value})")
        ...
        if highlight_hetu_states:
            ...
            for hetu_state in highlight_hetu_states[:3]:  # 最多显示3个
                ...
                lines.append(f"   {hetu_state.name} ({hetu_state.value}): {len(states)} 个卦象")
                if states:
                    # 显示该状态的代表性卦象
                    sample_state = states[0]
                    sample_mapping = self._by_binary[sample_state]
                    active_dims = [
                        dim for dim, val in sample_mapping.dimension_values.items() if val == 1
                    ]
                    lines.append(f"     示例: {sample_mapping.hexagram_name} ({sample_state})")
        ...
        if self.state_history:
            ...
            lines.append(f"   河图变化: {recent.from_hetu.name} → {recent.to_hetu.name}")
    ...
    try:
        ...
        if path:
            ...
            for i, state in enumerate(path):
                ...
                print(f"     步骤{i}: {state} ({manager.get_hexagram_name(state)}) - {hetu_name}")
        else:
