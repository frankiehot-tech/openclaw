...
def add_imports_after_block(filepath: Path, import_lines: list[str], marker: str = "from openclaw_roots import"):
...
from .config import load_plan_config"""
    ...
    if openclaw_end < 0:
        print(f"  WARNING: could not find openclaw_roots import end in executor.py")
        return
    ...
    if first_func < 0:
        print(f"  WARNING: could not find first function in executor.py")
        return
    ...
    filepath.write_text("\n".join(cleaned))
    print(f"  REPLACED: executor.py import block")
