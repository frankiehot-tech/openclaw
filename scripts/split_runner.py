...
MODULES = {
    "utils": {
        "functions": [
            "now_iso", "slugify", "clip", "read_json", "write_json",
            "extract_referenced_paths", "is_pid_alive",
            "terminate_process_tree", "terminate_pid_tree",
            "is_instruction_under_plan_dir",
            "system_free_memory_percent", "system_load_average",
            "ollama_active_cpu_percent",
            "extract_structured_result", "codex_executable",
    ...
    "config": {
        "functions": [
            "load_plan_config", "load_control_plane_config",
    ...
    "trace": {
        "functions": [
            "create_task_workspace", "update_trace_event",
            "update_trace_status_change", "add_trace_artifact",
        ],
        "needs_imports": ["now_iso", "read_json", "write_json"],
    },
    "state": {
        "functions": [
            "emit_event", "record_performance_metric",
        ],
        "needs_imports": [],
    ...
    "route_state": {
        "functions": [
            "route_runner_mode", "route_state_path", "route_state_lock_path",
            "_normalize_route_state", "load_route_state", "write_route_state",
            "route_current_item_ids", "mutate_route_state",
        ],
        "needs_imports": [
    ...
    "manifest": {
        "functions": [
            "load_manifest_items", "manifest_item_depends_on",
            "materialize_route_items", "compute_route_counts_and_status",
            "update_manifest_instruction_path",
            "archive_instruction_path_if_needed",
            "upsert_manifest_item",
            "normalize_generated_queue_item",
            "upsert_route_state_item", "append_generated_queue_items",
            "find_manifest_item", "active_route_item_ids",
            "find_manifest_item_with_normalization",
            "route_index_by_item_id",
        ],
        "needs_imports": [
            "read_json", "write_json", "slugify", "now_iso",
        ],
        "extra_imports": [
    ...
    "task": {
        "functions": [
            "load_tasks_payload", "save_tasks_payload",
            "upsert_task_record", "set_task_status",
            "set_route_item_state",
            "add_route_current_item", "remove_route_current_item",
            "replace_route_current_items",
            "reset_failed_item_for_auto_retry",
        ],
        "needs_imports": [
            "now_iso", "read_json", "write_json", "clip",
        ],
        "extra_imports": [
    ...
    "preflight": {
        "functions": [
            ...
            "build_preflight_warnings", "review_preflight_warnings",
            "plan_preflight_warnings", "validate_build_preflight",
            "render_prompt", "render_review_prompt", "render_plan_prompt",
        ],
        "needs_imports": [
            "extract_referenced_paths", "load_control_plane_config", "codex_executable",
        ],
        "extra_imports": [
    ...
    "failure": {
        "functions": [
            "failure_text", "retry_window_open",
            "is_retryable_failed_item", "is_blocked_rescue_retryable_failed_item",
            "failure_markdown", "success_markdown",
            "mark_stale_failed",
            "auto_retry_blocking_failures",
        ],
        "needs_imports": [
            "now_iso", "clip", "failure_text",
        ],
        "extra_imports": [
    ...
    "executor": {
        "functions": [
            "spawn_build_worker",
            "execute_build_item", "execute_review_item", "execute_plan_item",
            "execute_item",
            "append_to_daily_memory", "queue_route_by_mode",
            "detect_and_cleanup_stale_runs",
            "maybe_mark_restarted_runs_failed",
            "load_dependency_state_index",
            "route_matches_runner_modes", "choose_next_item",
            "finalize_completed_instruction",
            "archive_existing_completed_instructions",
        ],
        "needs_imports": [
            "now_iso", "clip", "slugify", "read_json", "write_json",
            "extract_structured_result", "codex_executable",
            "terminate_process_tree", "is_pid_alive", "resource_gate_message",
            "root_task_id_for",
        ],
        "extra_imports": [
...
MAIN_FILE_FUNCTIONS = {
    "handle_signal", "find_config_route", "main",
    "run_item_mode", "run_once_mode", "status_mode", "daemon_mode",
}
...
def generate_module(mod_name: str, mod_info: dict, func_ranges: dict, source_lines: list[str]) -> str | None:
    lines = []
...
def generate_main_file(source_lines: list[str], func_ranges: dict) -> str:
    ...
    lines.append('to `.openclaw/plan_queue` plus task artifacts under `.openclaw/orchestrator/tasks`.')
    lines.append('')
    ...
    for name in ["handle_signal", "find_config_route", "main", "run_item_mode", "run_once_mode", "status_mode", "daemon_mode"]:
        if name in func_ranges:
            start, end = func_ranges[name]
            code = "\n".join(source_lines[start:end])
            lines.append(code)
...
def main():
    ...
    if dry_run:
        print(f"  [DRY-RUN] Updated main file: {len(source_lines)} -> {len(main_content.split(chr(10)))} lines")
    else:
        out_path.write_text(main_content)
        print(f"  Wrote updated {out_path.name} ({len(main_content.split(chr(10)))} lines, was {len(source_lines)})")
