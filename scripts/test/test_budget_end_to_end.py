...
def test_end_to_end():
    ...
    try:
        ...
        assert abs(state.remaining - (initial_remaining - 30.0)) < 0.01, "剩余预算计算错误"
        ...
        # 检查预算引擎可用性标志
        assert hasattr(orchestrator, "BUDGET_ENGINE_AVAILABLE"), (
            "编排器应定义BUDGET_ENGINE_AVAILABLE"
        )
        assert orchestrator.BUDGET_ENGINE_AVAILABLE, "预算引擎应可用"

        print("   ✅ Athena编排器集成检查通过")
        ...
        assert success, f"任务创建应成功，实际失败: {task_id}"
