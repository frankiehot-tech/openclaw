...
def test_stage_registry_extension():
    ...
    for stage_id in high_risk_stages:
        ...
        # 检查字段类型
        assert isinstance(stage_info["evidence_requirements"], list), (
            f"{stage_id} evidence_requirements 不是列表"
        )
        assert isinstance(stage_info["validation_rule_set"], list), (
...
def test_validation_engine_basic():
    ...
    assert result2.decision == ValidationDecision.NEEDS_REVISION.value, (
    ...
    for rel_path in yaml_files:
        ...
        with open(abs_path, encoding="utf-8") as f:
            try:
                ...
                raise AssertionError(f"YAML 解析失败 {rel_path}: {e}") from e
