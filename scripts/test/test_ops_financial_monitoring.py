...
def test_financial_monitor_threshold():
    ...
    try:
        ...
        if daily_over_alerts:
            print("  ✗ 错误触发了每日预算超支警告（不应触发）")
        else:
            print("  ✓ 未触发每日预算超支警告（正确）")
...
def test_alert_dispatcher_dry_run():
    ...
    try:
        ...
        if dispatcher.config.dry_run:
            print("  ✓ 处于dry-run模式，告警未真实发送")

            # 检查日志文件是否真的创建（不应该）
            log_file = Path(config.log_file_path)
            if log_file.exists():
                # 在dry-run模式下，日志文件可能仍然会被创建（取决于实现）
                print("    注: 日志文件存在（可能由之前的测试创建）")
            else:
                print("    ✓ 日志文件未创建（dry-run行为正确）")
        ...
        if not any(results2.values()):
            print("  ✓ 低优先级告警被正确过滤（无渠道接收）")
        else:
            print("  ✗ 低优先级告警未被过滤（某些渠道接收了）")
