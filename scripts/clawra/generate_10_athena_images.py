...
class AthenaImageGenerator:
        ...
        for (var i = 0; i < editables.length; i++) {{
            ...
            if (elem.offsetParent !== null) {{
                ...
                if (found) {{
                    ...
                    elem.textContent = '';
                    // 输入新内容 - 使用正确转义的字符串
                    elem.textContent = {escaped_prompt};
            ...
            if result.success and result.output and result.output != "missing value":
                try:
                    ...
                    if current_count > last_image_count:
                        ...
                        for img in images:
                            if img.get("src") not in [
                                i.get("src", "") for i in self.generated_images
                            ]:
                                self.generated_images.append(img)
                                print(f"  新图像URL: {img.get('src', '')[:80]}...")
    ...
    def generate_all_variants(self):
        ...
        # 生成每个变体
        success_count = 0
        for i, variant in enumerate(athena_variants):
            print(f"\n{'='*60}")
        ...
        print(f"📊 生成完成总结")
        print(f"{'='*60}")
        print(f"   总尝试变体数: {len(athena_variants)}")
        ...
        if self.generated_images:
            print(f"\n📋 生成的图像:")
            for i, img in enumerate(self.generated_images[:10]):  # 最多显示10张
                print(f"   {i+1}. {img.get('src', '')[:100]}...")
