...
class HetuToHexagramAdapter:
    ...
    def transition(self, task_id: str, current_state: HetuState, target_state: HetuState) -> bool:
        ...
        # 计算汉明距离
        distance = self.hexagram_manager.hamming_distance(current_hexagram, target_hexagram)
    ...
    def load_states(self) -> None:
        ...
        try:
            ...
            for task_id, state_data in data.get("task_states", {}).items():
                hetu_history = [HetuState(value) for value in state_data.get("hetu_history", [])]
                timestamps = [datetime.fromisoformat(ts) for ts in state_data.get("timestamps", [])]
