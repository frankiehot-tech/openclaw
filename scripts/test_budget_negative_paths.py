...
def test_budget_exhaustion():
    ...
    try:
        ...
        # 验证模式转换到 low
        assert (
            state.current_mode == BudgetMode.LOW
        ...
        assert (
            result_low.decision != BudgetDecision.APPROVED
        ...
        # 验证模式转换到 critical
        assert (
            state.current_mode == BudgetMode.CRITICAL
        ...
        # 非核心、非允许任务类型的任务应该被拒绝
        assert (
            result_critical.decision != BudgetDecision.APPROVED
        ...
        # 验证模式转换到 paused
        assert (
            state.current_mode == BudgetMode.PAUSED
        ...
        assert (
            result_paused.decision != BudgetDecision.APPROVED
