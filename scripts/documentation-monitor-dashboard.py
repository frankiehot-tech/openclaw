...
class DocumentationMonitor:
                ...
                with open(latest_report, 'r', encoding='utf-8') as f:
                    ...
                    if match:
                        ...
                        if total_links_estimate > 0:
                            metrics['link_effectiveness'] = 100.0 * (total_links_estimate - broken_links) / total_links_estimate
    ...
    def _generate_alerts(self, metrics):
        ...
        # 可读性告警
        readability_threshold = thresholds.get('readability_analysis', {}).get('min_average_score', 65)
        if metrics['readability_average'] > 0 and metrics['readability_average'] < readability_threshold:
            alerts.append({
                'severity': 'warning',
        ...
        <script>
            ...
            fetch('/api/trends')
                ...
                .then(data => {{
                    ...
                    if (data.link_effectiveness && data.link_effectiveness.length > 0) {{
                        chartDiv.innerHTML = `<p>显示最近${data.link_effectiveness.length}天的链接有效性趋势数据</p>
                                            <p>最新值: ${data.link_effectiveness[0].value}%</p>
