...
Modes:
- daemon (default): continuously poll routes and execute items.
- run-once <queue_state_path>: execute the current item of a specific queue state file,
  wait for completion, then exit.
- status <queue_state_path>: show current status of a queue.
"""
...
# Parallel build gate for resource admission and isolation
try:
            ...
            return type(
                ...
                (),
                {
                    "decision": (
                        ...
                        else "DEGRADED" if budget >= 1 else "REJECTED"
                    ),
...
# Register a simple logging hook if event bus is available
if EVENT_BUS_AVAILABLE:
    try:
        bus = get_bus()
        # Simple hook that logs events to a file for verification
        log_path = RUNTIME_ROOT / ".openclaw" / "event_bus_log.jsonl"
        ...
        # Register for all hook points
        bus.register_hook(HookPoint.PRE_TOOL, log_hook)
        bus.register_hook(HookPoint.POST_TOOL, log_hook)
        bus.register_hook(HookPoint.TASK_START, log_hook)
        bus.register_hook(HookPoint.TASK_FINISH, log_hook)
        bus.register_hook(HookPoint.ARTIFACT_WRITTEN, log_hook)
    except Exception:
        pass
...
HEARTBEAT_TIMEOUT_SECONDS = int(
    os.getenv("ATHENA_AI_PLAN_HEARTBEAT_TIMEOUT_SECONDS", "30")
)  # 优化: 从5分钟减少到30秒 (与ProcessLifecycleContract的30秒心跳间隔保持一致)
STALE_TASK_TIMEOUT_SECONDS = int(
    os.getenv("ATHENA_AI_PLAN_STALE_TASK_TIMEOUT_SECONDS", "60")
)  # 优化: 从10分钟减少到5分钟，与心跳优化保持一致
...
CODEX_APP_PATH = Path("/Applications/Codex.app/Contents/Resources/codex")
...
def detect_and_cleanup_stale_runs(routes: list[dict[str, Any]]) -> None:
    ...
    for route in routes:
        ...
        for item_id, state_item in items.items():
            ...
            # Artifact absence alone is not a staleness indicator; rely on heartbeat and update time checks above
...
def execute_build_item(route: dict[str, Any], item: dict[str, Any]) -> None:
    ...
    command = [
        "/Users/frankie/bin/opencode-athena",
        "run",
        "--format",
        "default",
        "--title",
    ...
    while True:
        ...
        if (time.time() - last_update) >= 5:
            ...
            summary = clip(
                latest
                or (
                    f"OpenCode 正在执行，已运行 {int(elapsed)}s，"
    ...
    while True:
        ...
        if (time.time() - last_update) >= 5:
            ...
            summary = clip(
                latest
                or (
                    f"Codex 正在审计，已运行 {int(elapsed)}s，"
                    f"最近输出约 {int(silent_for)}s 前。"
                )
            )
            add_route_current_item(route, item_id)
